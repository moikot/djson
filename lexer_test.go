...
func Test_Lex_Succeeds(t *testing.T) {
	testCases := []lexTestCase{
		...
		newTestCase("escaping a single quote ' in a string", "key='val1\\'val2'",
			[]token{
				newToken(tokenMapKey, 0, "key"),
				newToken(tokenAssignment, 3, "="),
				newToken(tokenString, 4, "'val1'val2'"),
		...
		newTestCase("escaping a backslash \\ in a string", "key='val1\\\\val2'",
			[]token{
				newToken(tokenMapKey, 0, "key"),
				newToken(tokenAssignment, 3, "="),
				newToken(tokenString, 4, "'val1\\val2'"),
		...
		newTestCase("escaping unescapable in a string", "key='val1\\-val2'",
			[]token{
				newToken(tokenMapKey, 0, "key"),
				newToken(tokenAssignment, 3, "="),
				newToken(tokenError, 4, "unknown escape sequence: character: U+002D '-'"),
			}),
		...
		newTestCase("escaping a backslash \\ in a value", "key=part1\\\\part2",
			[]token{
				newToken(tokenMapKey, 0, "key"),
				newToken(tokenAssignment, 3, "="),
				newToken(tokenValue, 4, "part1\\part2"),
